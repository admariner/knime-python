...
/**
 * {@link KaiNodeInterface} implementation for Python based nodes.
 *
 * @author Adrian Nembach, KNIME GmbH, Konstanz, Germany
 */
@SuppressWarnings("restriction")
final class ExtensionKaiNodeInterface implements KaiNodeInterface {

    private final Supplier<NodeDialogProxy> m_proxyProvider;

    private String m_extensionVersion;

    ExtensionKaiNodeInterface(final Supplier<NodeDialogProxy> proxyProvider, final String extensionVersion) {
    ...
    public ConfigurePrompt getConfigurePrompt(final Map<SettingsType, NodeAndVariableSettingsRO> settings,
        ...
        try (var proxy = m_proxyProvider.get()) {
            ...
            var jsonSettings = proxy.getSettingsSchema(JsonNodeSettingsSchema.readVersion(modelSettings))
                .createFromSettings(modelSettings);
            var schema = proxy.getSettingsSchema(m_extensionVersion);
            var systemMessage = DefaultKaiNodeInterface.constructSystemMessage(jsonSettings.getParameters(), specs);
            return new ConfigurePrompt(systemMessage, schema.getSchema());
        }
    }
    ...
    public void applyConfigureResponse(final String response,
        ...
        try (var proxy = m_proxyProvider.get()) {
            var schema = proxy.getSettingsSchema(m_extensionVersion);
            var data = JsonNodeSettingsMapperUtil.getNestedJsonObject(response, "data");
            schema.createFromJson(data).saveTo(settings.get(SettingsType.MODEL));
        }
    }
