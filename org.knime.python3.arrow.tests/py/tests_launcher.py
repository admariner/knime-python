...
def structcomplex_test_value(r, b):
    ...
    string_list = [
        "r:{},b:{},i:{}".format(r, b, i) if i % 7 != 0 else None
...
TEST_VALUES = {
    ...
    "varbinary": lambda r, b: bytes([(b + i) % 128 for i in range(r % 10)]),
    "void": lambda r, b: None,
    "struct": lambda r, b: {"0": r + b, "1": "Row: {}, Batch: {}".format(r, b)},
    "structcomplex": structcomplex_test_value,
    "list": lambda r, b: [b + r + i for i in range(r % 10)],
    "string": lambda r, b: "Row: {}, Batch: {}".format(r, b),
...
class EntryPoint(kg.EntryPoint):
    def testTypeToPython(self, data_type, data_source):
        ...
        def check_batch(batch, b):
            ...
            assert (
                len(array) == NUM_ROWS
            ), "Array has the wrong length. Expected {} got {}.".format(
                NUM_ROWS, len(array)
            )
            # Check array type
            assert isinstance(
                ...
                expected_array_type, type(array)
            )
            for r, v in enumerate(array):
                if r % 13 == 0:
                    ...
                    ), "Value has wrong type. Expected '{}', got '{}'".format(
                        expected_value_type, v.type
                    )
                    # Check value
                    assert_value(r, b, v)
        ...
        with kg.map_data_sink(data_sink) as sink:
            ...
            for b in range(num_batches):
                ...
                string_data = pa.array(
                    ["{},{}".format(i, b) for i in range(num_rows)], type=pa.string()
                )
